...
H_values = [2, 5, 15, 30, 50, 75]; % Diferentes horizontes a testar
R_fixed = 0.1;             % R fixo
...
p_y_abs = gobjects(1, length(H_values));
p_u_abs = gobjects(1, length(H_values));
p_y_inc = gobjects(1, length(H_values));
p_u_inc = gobjects(1, length(H_values));
...
% Arrays to store computation times for the table
avg_mpc_times = zeros(1, length(H_values));
max_mpc_times = zeros(1, length(H_values));
...
for i = 1:length(H_values)
    ...
    % Estado Inicial
    x(:,1) = Dx0 + x_ss;
    % Initialize array to store times for this specific H
    time_per_step = nan(1, N_sim-1);
    ...
    for k = 1:N_sim-1
        ...
        Dy(:,k) = y(:,k) - y_ss;
        Dx(:,k) = x(:,k) - x_ss;
        % --- TIMING THE MPC SOLVER ---
        tic; % Start timer
        % Chamar função do MPC NÃO RESTRINGIDO (Questão 4.2)
        Du(:,k) = mpc_solve(Dx(:,k), H, R, A, B, C); 
        time_per_step(k) = toc; % Stop timer and record
        u(:,k) = u_ss + Du(:,k);
        % Applies the control variable to the plant
        x(:,k+1) = h1(x(:,k),u(:,k));
    end
    
    % Calculate average and max times for this H
    avg_mpc_times(i) = mean(time_per_step);
    max_mpc_times(i) = max(time_per_step);
...
fprintf('\n--- MPC Computation Time vs Horizon (H) ---\n');
fprintf('  H  | Avg Time (s) | Max Time (s) | Limit (s)\n');
fprintf('----------------------------------------------\n');
for i = 1:length(H_values)
    fprintf(' %-3d |   %.6f   |   %.6f   |   0.50  \n', H_values(i), avg_mpc_times(i), max_mpc_times(i));
end
fprintf('----------------------------------------------\n');
fprintf('Note: Target is < 10%% of Ts (0.5 seconds).\n\n');